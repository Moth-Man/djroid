...
## Vision
DJroid is a DJ-centric cli toolsuite that provides DJs a way to interact with their archive of music files, giving them the ability to pass a prompt to the command line describing a certain criteria about the music they wish to play, and djroid generating a rekordbox-recognizable playlist using a combination of key-words from that prompt and internal algorithms/prompts relating to the art and science of DJing, generating a set of queries that pull from an internal database of music (previously created by Djroid) to assemble said playlist.
...
- crate: convert a passed-in prompt from the user into a series of queries that provide a list of songs meeting the criteria of that prompt (list of songs)